...
public class Bolsa 
{
    ...
    {
        if (obj.getPeso() > this.PESO_MAXIMO || this.porNombre.containsValue(obj))
        {
            System.out.println (this.nombre + ": No se puede agregar " + obj);
        }
        else 
        {
    ...
    public Elemento delElemento(String nombre) 
    {
        ...
        {
            Elemento temp = this.porNombre.get(nombre);
            this.pesoActual -= this.porNombre.get(nombre).getPeso();
    ...
    {
        ArrayList <Elemento> elementosAlmacenados = new ArrayList <Elemento>();
        for (Map.Entry <String, Elemento> entry: this.porNombre.entrySet())
        {
             elementosAlmacenados.add(entry.getValue());
        }
    ...
    public ArrayList<Elemento> getElementosConPrefijo(String pre)
    {
        ...
        for (Map.Entry <String,Elemento> entry: this.porNombre.entrySet())
        {
            if (entry.getKey().trim().toLowerCase().startsWith(pre))
            {
                elementosConPrefijo.add(entry.getValue());
            }
        }
    ...
    {
        int pesoLibre = this.PESO_MAXIMO - this.pesoActual;
        return pesoLibre;
        // TODO - Implementar metodo
    }
